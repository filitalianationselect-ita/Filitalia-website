(function () {
  ...
  const demoStore = clone(SEED);
  Object.keys(storedDemo || {}).forEach(function (key) {
    if (Array.isArray(storedDemo[key])) demoStore[key] = storedDemo[key];
  });
  ...
  function addModeCard() {
    ...
    $("adminRealLoginBtn").onclick = function () { if (mode === "real") disconnectReal(); else openLoginModal(); };
  }
  ...
  function addDashboardCard() {
    ...
    target.insertAdjacentHTML("afterend", `<div id="lightDashboardCard" class="light-integration-card"><div class="light-integration-head"><div><b>Centro operativo evento</b><div class="muted">Registrazioni, pagamenti, documenti, presenze ed email nello stesso pannello.</div></div><div class="light-integration-actions"><button id="dashEventDay" class="btn primary">🎯 Event Day</button><button id="dashComms" class="btn secondary">✉️ Comunicazioni</button></div></div></div>`);
  ...
  function addGmailBridge() {
    ...
    const topbar = section.querySelector(".topbar");
    if (!topbar) return;
    topbar.insertAdjacentHTML("afterend", `<div id="gmailLightCard" class="light-integration-card gmail-light"><div class="gmail-light-state"><span id="gmailLightDot" class="gmail-light-dot"></span><div><b id="gmailLightTitle">Gmail FIL-ITALIA non collegato</b><div class="muted">Ogni email parte separatamente e gli indirizzi restano privati.</div></div></div><button id="gmailLightBtn" class="btn primary">Collega Gmail FIL-ITALIA</button></div>`);
    $("gmailLightBtn").onclick = connectGmail;
    if ($("cSend")) $("cSend").onclick = sendCommunication;
    if ($("cTest")) $("cTest").onclick = sendTestCommunication;
    paintGmail();
  }
  ...
  function buildEventDay() {
    ...
    d.body.insertAdjacentHTML("beforeend", `
      <div id="eventDayLight" class="eventday-overlay">
        <header class="eventday-top"><div><b>FIL-ITALIA · Event Day</b><div id="eventDaySub" class="muted"></div></div><div class="light-integration-actions"><span id="eventModeTop" class="real-mode-pill"><span class="real-mode-dot"></span>DEMO</span><button id="eventDayAdd" class="btn secondary">＋ Registrazione</button><button id="eventDayEmail" class="btn secondary">✉️ Comunicazioni</button><button id="eventDayExport" class="btn secondary">⇩ CSV</button><button id="eventDayClose" class="btn primary">Chiudi</button></div></header>
        <main class="eventday-body">
          <div class="eventday-stats"><div class="eventday-stat"><span>ISCRITTI</span><strong id="edTotal">0</strong></div><div class="eventday-stat"><span>PAGATI</span><strong id="edPaid">0</strong></div><div class="eventday-stat"><span>CERTIFICATI</span><strong id="edCertificates">0</strong></div><div class="eventday-stat"><span>PRESENTI</span><strong id="edPresent">0</strong></div></div>
          <section class="eventday-grid"><div class="eventday-panel"><div class="eventday-tools"><input id="edSearch" class="input" placeholder="Cerca giocatore, email o categoria"><div class="eventday-filters"><button class="active" data-ed-filter="all">Tutti</button><button data-ed-filter="pending">Da pagare</button><button data-ed-filter="certificate">Senza certificato</button><button data-ed-filter="present">Presenti</button><button data-ed-filter="U12">U12</button><button data-ed-filter="U14">U14</button><button data-ed-filter="U16">U16</button><button data-ed-filter="U18">U18</button><button data-ed-filter="U19">U19</button></div></div><div id="edList" class="eventday-list"></div></div><div id="edDetail" class="eventday-panel"></div></section>
      ...
      <div id="registrationLightModal" class="light-modal"><div class="light-modal-card"><h2 id="registrationLightTitle">Nuova registrazione</h2><div class="muted">I dati vengono salvati nell’evento selezionato.</div><div class="light-form"><label>Nome e cognome<input id="lfName"></label><label>Anno<input id="lfYear" maxlength="4"></label><label>Categoria<select id="lfCat"><option>U12</option><option>U14</option><option>U16</option><option>U18</option><option>U19</option></select></label><label>Taglia maglia<select id="lfShirt"><option>Nessuna</option><option>XS</option><option>S</option><option>M</option><option>L</option><option>XL</option><option>XXL</option></select></label><label class="full">Email<input id="lfEmail" type="email"></label><label>Telefono<input id="lfPhone"></label><label>Genitore / riferimento<input id="lfParent"></label></div><div class="light-modal-actions"><button id="registrationLightCancel" class="btn secondary">Annulla</button><button id="registrationLightSave" class="btn primary">Salva</button></div></div></div>
    ...
    $("eventDayExport").onclick = function () { if (currentService()) currentService().exportCsv(data, "filitalia-" + currentEvent.city.toLowerCase() + "-registrazioni.csv"); };
    ...
    [$("realLoginModal"), $("registrationLightModal"), $("paymentLightModal")].forEach(function (modal) { modal.onclick = function (event) { if (event.target === modal) modal.classList.remove("show"); }; });
  }

  function paintMode() {
    const real = mode === "real";
    [$("adminDataModePill"), $("eventModeTop")].forEach(function (item) {
      if (!item) return;
      item.classList.toggle("real", real);
      item.innerHTML = '<span class="real-mode-dot"></span>' + (real ? "DATI REALI" : "DEMO");
    });
    if ($("adminDataModeText")) $("adminDataModeText").textContent = real ? "Collegato a Supabase. Le modifiche vengono salvate realmente." : "Modalità demo attiva. Puoi provare tutto senza modificare i dati reali.";
  ...
  async function detectMode() {
    ...
    try { await service.requireAdmin(); mode = "real"; }
    catch (_) { mode = "demo"; }
    paintMode();
  }
  ...
  async function loadCurrentEvent() {
    ...
    if (mode === "real" && currentService()) {
      try { data = await currentService().loadEvent(currentEvent.id); }
      catch (error) { mode = "demo"; paintMode(); notify(friendly(error)); data = clone(demoStore[currentEvent.id] || []); }
    } else data = clone(demoStore[currentEvent.id] || []);
  ...
  function visiblePlayers() {
    ...
    return data.filter(function (player) {
      const filterMatch = filter === "all" || filter === player.cat || (filter === "pending" && !isComplete(player)) || (filter === "certificate" && !player.certificate) || (filter === "present" && player.present);
      return filterMatch && [player.name, player.email, player.cat, player.year, player.parent, player.shirt].join(" ").toLowerCase().includes(query);
    });
  }
  ...
  function renderEventDay() {
    ...
    $("edList").innerHTML = rows.length ? rows.map(function (player) {
      return `<button class="eventday-player ${player.id === selected ? "active" : ""}" data-ed-id="${esc(player.id)}"><span class="eventday-pill ${isComplete(player) ? "done" : ""}">${isComplete(player) ? "Pagamento ok" : "Da pagare"}</span><b>${esc(player.name)}</b><br><small>${esc(player.cat)} · ${esc(player.year)} · Taglia ${esc(player.shirt || "—")}</small></button>`;
    }).join("") : '<div class="muted" style="padding:20px;text-align:center">Nessun partecipante trovato.</div>';
    d.querySelectorAll("[data-ed-id]").forEach(function (button) { button.onclick = function () { selected = button.dataset.edId; renderEventDay(); }; });
    ...
    if (!player) {
      ...
      d.querySelectorAll("[data-ed-task]").forEach(function (button) { button.onclick = function () { toggleTask(player, button.dataset.edTask); }; });
    }
    $("edTotal").textContent = data.length;
    $("edPaid").textContent = data.filter(isComplete).length;
    $("edCertificates").textContent = data.filter(function (player) { return player.certificate; }).length;
    $("edPresent").textContent = data.filter(function (player) { return player.present; }).length;
  }
  ...
  function openLoginModal() { $("realLoginPassword").value = ""; $("realLoginModal").classList.add("show"); setTimeout(function () { $("realLoginEmail").focus(); }, 50); }
  async function submitRealLogin() {
    ...
    } catch (error) { mode = "demo"; paintMode(); notify(friendly(error)); }
    finally { button.disabled = false; button.textContent = "Accedi"; }
  }
  async function disconnectReal() { try { if (window.FilitaliaAuth) await window.FilitaliaAuth.signOut(); } catch (_) {} mode = "demo"; paintMode(); await loadCurrentEvent(); notify("Modalità demo riattivata."); }

  async function paintGmail() {
    if (!$("gmailLightDot")) return;
    let connected = false, address = "";
    if (mode === "real" && currentService()) {
      try { const connection = await currentService().getGmailConnection(); connected = Boolean(connection); address = connection ? connection.gmail_address : ""; }
      catch (_) { connected = false; }
    } else connected = localStorage.getItem(GMAIL_KEY) === "true";
  ...
  async function connectGmail() {
    if (mode === "real" && currentService()) {
      try { const connection = await currentService().getGmailConnection(); if (connection) { notify("Gmail FIL-ITALIA è già collegato."); return; } await currentService().startGmailConnection(); }
      catch (error) { notify(friendly(error)); }
      return;
    }
  ...
  async function sendTestCommunication() {
    ...
    if (mode === "real" && currentService()) {
      try { const result = await currentService().sendEmail({ event_id: currentEvent.id, subject: "TEST · " + subject, body_template: body, recipients: [{ email: email, name: "Test FIL-ITALIA" }] }); notify((result.sent || 0) + " email di test inviata."); }
      catch (error) { notify(friendly(error)); }
      return;
    }
