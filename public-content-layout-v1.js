...
function applyLayout(list){records=Array.isArray(list)?list:[];const map=new Map(records.map(row=>[key(String(row.content_type||row.contentType||''),String(row.item_id||row.itemId||'')),row]));
 ['player','staff','news','event'].forEach(function(type){const items=source(type);if(!items.length)return;items.forEach(function(item,index){const id=itemId(type,item,index);const row=map.get(key(type,id));item.__filitaliaId=id;item.displayOrder=numeric(row&&(row.display_order??row.displayOrder),numeric(item.displayOrder,1000+index));item.featured=Boolean(row?row.featured:item.featured);item.homeSection=String(row&&(row.home_section??row.homeSection)||item.homeSection||'default')});
  items.sort(function(a,b){
   if(type==='event')return dateValue(a)-dateValue(b)||text(a.title).localeCompare(text(b.title),'it');
   if(type==='news'){
    if(Boolean(a.featured)!==Boolean(b.featured))return a.featured?-1:1;
    const order=numeric(a.displayOrder,1000)-numeric(b.displayOrder,1000);if(order)return order;
    return String(b.sortDate||b.publishDate||'').localeCompare(String(a.sortDate||a.publishDate||''));
   }
   if(type==='staff'){
    const group=function(item){return item.homeSection==='leadership'?0:item.homeSection==='team'?1:2};
    const section=group(a)-group(b);if(section)return section;
   }
   if(type==='player'&&Boolean(a.featured)!==Boolean(b.featured))return a.featured?-1:1;
   return numeric(a.displayOrder,1000)-numeric(b.displayOrder,1000)||text(a.name||a.title).localeCompare(text(b.name||b.title),'it');
  });
 });
 rerender();
}

function rerender(){window.clearTimeout(refreshTimer);refreshTimer=window.setTimeout(function(){
 ['renderHomePlayers','renderPlayersPage','renderHomeStaff','renderStaffPage','renderNews','renderEvents'].forEach(function(name){try{if(typeof window[name]==='function')window[name]()}catch(error){console.warn('FIL-ITALIA layout render',name,error)}});
 window.dispatchEvent(new CustomEvent('filitalia:content-order-updated',{detail:{records:records.slice()}}));
},20)}

async function loadRemote(){if(!cfg.supabaseUrl||!cfg.supabasePublishableKey||!window.supabase)return null;const client=window.supabase.createClient(cfg.supabaseUrl,cfg.supabasePublishableKey,{auth:{persistSession:false,autoRefreshToken:false}});const result=await client.from('admin_content_layout').select('content_type,item_id,display_order,featured,home_section').order('content_type').order('display_order');if(result.error)throw result.error;return result.data||[]}

function refresh(){const local=readLocal();if(local.length)applyLayout(local);else applyLayout(records);loadRemote().then(function(remote){if(remote&&remote.length)applyLayout(remote)}).catch(function(error){console.info('Ordine contenuti remoto non ancora disponibile',error&&error.message?error.message:error)})}

window.addEventListener('filitalia:public-content-updated',function(){window.setTimeout(function(){applyLayout(records.length?records:readLocal())},30)});
window.addEventListener('filitalia:layout-updated',function(event){applyLayout(event&&event.detail&&event.detail.records||readLocal())});
window.addEventListener('storage',function(event){if(!event.key||event.key===STORAGE_KEY)applyLayout(readLocal())});
