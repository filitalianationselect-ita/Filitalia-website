...
-- Additive migration: preserves the existing profiles, player_profiles,
-- camp_registrations and public_player_cards tables.
-- The new registry becomes the long-term source of truth while legacy data
-- can be migrated and verified without destructive updates.
...
create index if not exists players_legacy_profile_idx on public.players(legacy_profile_id);
...
create table if not exists public.program_events (
  ...
  end_at timestamptz,
  venue text,
  event_type text not null default 'camp' check (event_type in ('camp','tournament','training','tryout','selection','travel','other')),
  status text not null default 'active' check (status in ('draft','active','completed','archived','cancelled')),
...
create index if not exists player_event_registrations_event_idx
  on public.player_event_registrations(event_id, created_at desc);
create index if not exists player_event_registrations_payment_idx
  on public.player_event_registrations(payment_status);
create index if not exists player_event_registrations_selection_idx
  on public.player_event_registrations(selection_status);
...
create table if not exists public.player_evaluations (
  ...
  recommendation text check (recommendation is null or recommendation in ('monitor','invite','select','not_select','priority')),
...
create index if not exists player_evaluations_event_idx
...
create table if not exists public.player_notes (
  ...
  note_type text not null default 'general' check (note_type in ('general','scouting','medical_admin','eligibility','communication','follow_up')),
  body text not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
-- ---------------------------------------------------------------------------
-- Player Cards v2: keyed by canonical player, therefore also supports minors
-- without their own login account.
-- ---------------------------------------------------------------------------
...
using (
  ...
  or public.filitalia_has_active_role(array['coach','coordinator','staff']::text[])
);

drop policy if exists player_account_links_own_read on public.player_account_links;
create policy player_account_links_own_read on public.player_account_links
for select to authenticated
using (account_id = auth.uid() or public.is_active_admin());

drop policy if exists program_events_authenticated_read on public.program_events;
create policy program_events_authenticated_read on public.program_events
for select to authenticated
using (true);
...
drop policy if exists evaluations_staff_read on public.player_evaluations;
create policy evaluations_staff_read on public.player_evaluations
for select to authenticated
using (
  public.is_active_admin()
  or public.filitalia_has_active_role(array['coach','coordinator','staff']::text[])
...
drop policy if exists notes_staff_read on public.player_notes;
create policy notes_staff_read on public.player_notes
for select to authenticated
using (
  public.is_active_admin()
  or public.filitalia_has_active_role(array['coach','coordinator','staff']::text[])
...
drop policy if exists documents_admin_read on public.player_documents;
create policy documents_admin_read on public.player_documents
for select to authenticated
using (
  public.is_active_admin()
  or exists (
...
drop policy if exists public_player_cards_v2_read_public on public.public_player_cards_v2;
create policy public_player_cards_v2_read_public on public.public_player_cards_v2
...
-- ---------------------------------------------------------------------------
-- Convert an existing player account into one canonical player record.
-- This is idempotent and does not delete the legacy profile.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if account_profile.id is null then
...
begin
  ...
  if caller_profile.id is null or caller_profile.status <> 'active' or caller_profile.role <> 'player' then
...
begin
  if auth.uid() is null then raise exception 'NOT_AUTHENTICATED'; end if;
  select * into caller_profile from public.profiles where id = auth.uid();
  if caller_profile.id is null or caller_profile.status <> 'active' or caller_profile.role not in ('parent','admin') then
  ...
  new_identity := public.filitalia_manual_identity_key(
    player_data->>'first_name',
    player_data->>'last_name',
    birth_value
  );

  if new_identity is null then raise exception 'PLAYER_IDENTITY_REQUIRED'; end if;

  select * into existing_player from public.players where identity_key = new_identity;
  if existing_player.id is not null then
    if exists (
      select 1 from public.player_account_links
      where player_id = existing_player.id and account_id = auth.uid()
    ) then
  ...
  insert into public.players (
    ...
    case when player_data ? 'italian_passport' then (player_data->>'italian_passport')::boolean else null end,
    case when player_data ? 'filipino_passport' then (player_data->>'filipino_passport')::boolean else null end,
...
-- ---------------------------------------------------------------------------
-- Service-only registration ingestion. Used by the register-camp Edge Function.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if canonical_player_id is not null and verified_account_id is not null then
    if not exists (
      select 1 from public.player_account_links l
  ...
  elsif canonical_player_id is not null and verified_account_id is null then
    canonical_player_id := null;
  end if;
  ...
  identity_value := nullif(coalesce(
    submission->>'Player Identity Key',
    submission->>'player_identity_key'
  ), '');
  ...
  event_external_value := nullif(trim(coalesce(submission->>'eventId', submission->>'event_id')), '');
  ...
  if event_external_value is null then
    event_external_value := 'legacy:' || md5(lower(event_name_value || '|' || coalesce(event_city_value,'') || '|' || coalesce(event_date_label_value,'')));
  end if;

  begin
    event_date_value := event_date_label_value::date;
  exception when others then
  ...
  insert into public.program_events (
    ...
    event_external_value,
    event_name_value,
    event_city_value,
    event_date_value,
    event_date_label_value,
    'camp',
    'active',
    jsonb_build_object('source','website_registration')
  )
  ...
  begin
    submission_uuid := nullif(submission->>'submissionId', '')::uuid;
  exception when others then
    submission_uuid := gen_random_uuid();
  end;
...
begin
  ...
  where p.status <> 'merged'
    and (search_term is null or trim(search_term) = '' or lower(
      concat_ws(' ', p.first_name, p.last_name, p.email, p.phone, p.residence_city, p.current_club, p.position)
...
begin
  ...
  select jsonb_build_object(
    'player', to_jsonb(p),
    'account_links', coalesce((
      select jsonb_agg(jsonb_build_object(
        'account_id', l.account_id,
        'relationship', l.relationship,
        'is_primary', l.is_primary,
        'email', pr.email,
        'name', trim(concat_ws(' ', pr.first_name, pr.last_name)),
        'role', pr.role,
        'status', pr.status
    ...
    'registrations', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'event_date_label', e.date_label,
        'registration_status', r.registration_status,
        'attendance_status', r.attendance_status,
        'selection_status', r.selection_status,
        'payment_status', r.payment_status,
        'shirt_size', r.shirt_size,
        'guardian_snapshot', r.guardian_snapshot,
    ...
    'evaluations', coalesce((
      select jsonb_agg(to_jsonb(ev) || jsonb_build_object('event_name', e.name, 'evaluator_name', trim(concat_ws(' ', pr.first_name, pr.last_name))) order by ev.created_at desc)
    ...
    'payments', coalesce((
      select jsonb_agg(to_jsonb(pay) || jsonb_build_object('event_name', e.name) order by pay.created_at desc)
    ...
    'notes', coalesce((
      select jsonb_agg(to_jsonb(n) || jsonb_build_object('author_name', trim(concat_ws(' ', pr.first_name, pr.last_name))) order by n.created_at desc)
    ...
    'documents', coalesce((
      ...
      from public.player_documents d where d.player_id = p.id
    ), '[]'::jsonb),
...
create or replace function public.admin_update_registry_player(target_player_id uuid, patch jsonb)
returns public.players
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.players set
    ...
    last_name = coalesce(nullif(trim(patch->>'last_name'), ''), last_name),
    birth_date = coalesce(nullif(patch->>'birth_date', '')::date, birth_date),
...
begin
  ...
  from public.player_event_registrations
  where id = (evaluation_data->>'registration_id')::uuid;
  if target_registration.id is null then raise exception 'REGISTRATION_NOT_FOUND'; end if;

  insert into public.player_evaluations (
    player_id, registration_id, event_id, evaluator_id,
    skill, basketball_iq, defense, athleticism, mentality,
    recommendation, technical_notes, private_notes
  ) values (
    target_registration.player_id,
...
begin
  if not public.is_active_admin() then raise exception 'NOT_AUTHORIZED'; end if;
  payment_status_value := coalesce(nullif(payment_data->>'status',''), 'pending');

  insert into public.registration_payments (
    registration_id, amount_cents, currency, status, method,
    transaction_reference, paid_at, notes, recorded_by
  ) values (
    (payment_data->>'registration_id')::uuid,
    coalesce(nullif(payment_data->>'amount_cents','')::integer, 0),
    coalesce(nullif(payment_data->>'currency',''), 'EUR'),
    payment_status_value,
    nullif(payment_data->>'method',''),
    nullif(payment_data->>'transaction_reference',''),
    nullif(payment_data->>'paid_at','')::timestamptz,
    nullif(payment_data->>'notes',''),
    auth.uid()
  ) returning * into result_payment;

  update public.player_event_registrations
  set payment_status = payment_status_value, updated_at = now()
...
begin
  ...
  update public.program_events set
    ...
    public_visible = case when archive then false else public_visible end,
    updated_at = now()
...
declare
  target_player public.players;
  result_card public.public_player_cards_v2;
begin
  if not public.is_active_admin() then raise exception 'NOT_AUTHORIZED'; end if;
  select * into target_player from public.players where id = target_player_id and status = 'active';
  if target_player.id is null then raise exception 'PLAYER_NOT_FOUND'; end if;
  if coalesce(trim(target_player.position),'') = '' then raise exception 'PLAYER_CARD_POSITION_REQUIRED'; end if;
  if coalesce(trim(target_player.residence_city),'') = '' then raise exception 'PLAYER_CARD_CITY_REQUIRED'; end if;
  if coalesce(trim(target_player.photo_path),'') = '' then raise exception 'PLAYER_CARD_PHOTO_REQUIRED'; end if;

  insert into public.public_player_cards_v2 (
    player_id, full_name, birth_year, category, position, height_cm,
    current_club, city, nationality, instagram, highlights_url,
    photo_path, published_by, published_at, updated_at
  ) values (
    target_player.id,
    trim(concat_ws(' ', target_player.first_name, target_player.last_name)),
    ...
    target_player.current_club,
    target_player.residence_city,
    public.player_card_nationality(target_player.italian_passport, target_player.filipino_passport),
    target_player.instagram,
    target_player.highlights_url,
    target_player.photo_path,
    auth.uid(),
    now(),
    now()
  )
...
-- ---------------------------------------------------------------------------
-- Public compatibility view. Once the migration is live, the website can read
-- one source while older cards continue working until they are republished v2.
-- ---------------------------------------------------------------------------
...
select
  ...
  pc.photo_path,
  pc.published_at,
  'legacy_account'::text as source
from public.public_player_cards pc
left join public.players p on p.legacy_profile_id = pc.user_id
where not exists (
  select 1 from public.public_player_cards_v2 newer
  where newer.player_id = p.id
);

grant select on public.public_player_cards_unified to anon, authenticated;

-- Public photo access for v2 cards stored in the existing private bucket.
...
using (
  ...
  and exists (
    select 1 from public.public_player_cards_v2 pc
    where pc.photo_path = name
  )
);

-- ---------------------------------------------------------------------------
-- Existing account backfill. Only complete player profiles are inserted.
...
select
  ...
  pp.birth_date,
  pp.sex,
  coalesce(pp.residence_city, p.city),
  p.email,
  p.phone,
  pp.position,
  pp.current_club,
  ...
  'account_backfill'
from public.profiles p
join public.player_profiles pp on pp.user_id = p.id
where pp.birth_date is not null
  and (p.requested_role = 'player' or p.role = 'player')
on conflict (identity_key) do nothing;

insert into public.player_account_links(player_id, account_id, relationship, is_primary)
select pl.id, pl.legacy_profile_id, 'self', true
from public.players pl
where pl.legacy_profile_id is not null
on conflict (player_id, account_id) do nothing;
