  ...
  try{
    if(typeof newsData!=='undefined'&&Array.isArray(newsData)){
      newsData.sort(function(a,b){
        ...
        return featured||timeValue(b)-timeValue(a);
      });
    }
  }catch(_){}
}
function normalizeStandaloneNav(){
  ...
  if(!nav)return;
  nav.querySelectorAll('a[href*="account"]').forEach(function(link){link.remove()});
  const labels=[
    ['index.html','Home'],['index.html#about','Chi siamo'],['players.html','Giocatori'],['gallery.html','Media'],['staff.html','Staff'],['news.html','News'],['events.html','Eventi'],['camp-register.html','Camp'],['index.html#contact-modal','Contatti']
  ];
  nav.innerHTML=labels.map(function(pair){return'<a href="'+pair[0]+'">'+pair[1]+'</a>'}).join('');
}
function boot(){sortNews();normalizeStandaloneNav();document.body.classList.remove('mobile-menu-open')}
